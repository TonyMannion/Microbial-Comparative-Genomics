...
parser.add_argument('-d', '--download_data', dest='download_reports', default = 'no', help='Download genome reports, contigs, and annotations data for assembled/annotated genomes for previously completed jobs to output folder? Enter "yes" or "no". Default is "no".')
parser.add_argument('-p', '--patric_download', dest='patric_dl', default = 'yes', help='Download genome reports, contigs, and annotations data for PATRIC genomes? Enter "yes" or "no". Default is "yes".')
...
if str(args.patric_dl) == 'yes':
	...
	for genome_id,genome_name in zip(genome_ids_list,genome_name_list): 
		...
		os.system('p3-echo -t genome_id '+str(genome_id)+' | p3-get-genome-features --attr genome_name --attr patric_id  --attr pgfam_id  > '+str(args.output_folder)+'/'+str(genome_name)+'_annotation.txt')
		...
		df_genome2['genome_name']=str(genome_name)
