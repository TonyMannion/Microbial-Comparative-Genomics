...
def make_tree():
	...
	df_concat=pd.concat([pd.read_csv(str(args.input_folder)+'/'+str(genome_name)+'_OG_annotation.txt',sep='\t') for genome_name in genome_name_list])
	#groupby
	df_concat['count']=1
	df_groupby=df_concat.replace(' ','_',regex=True).groupby(['genome_name','Orthogroup'],as_index=False).sum().pivot(index='genome_name',columns='Orthogroup',values='count').fillna(0).astype(int)
...
def make_clustermap():
	...
	#groupby
	df_concat['count']=1
	df_groupby=df_concat.groupby(['genome_name','Orthogroup'],as_index=False).sum().pivot(columns='genome_name',index='Orthogroup',values='count').fillna(0)
	...
	df_cm2.to_csv(output_folder+'/'+'Orthogroup_clustermap_dataframe.txt',sep='\t')
...
def core_unique_genes():
	...
	#groupby
	df_concat['count']=1
	df_groupby=df_concat.groupby(['genome_name','Orthogroup'],as_index=False).sum().pivot(columns='genome_name',index='Orthogroup',values='count').fillna(0).to_csv(output_folder+'/'+'gene_family_groupby_out.txt',sep='\t') 
	#core
	...
	genome_name_list=df_genome_names['genome_name'].dropna().tolist()
	for genome in genome_name_list:
		df_an=pd.read_csv(str(args.input_folder)+'/'+str(genome)+'_OG_annotation.txt',sep='\t')
		df_merged=pd.merge(df_an,df_concat,left_on='Orthogroup',right_on='Orthogroup',how="left").to_csv(output_folder+'/'+str(genome)+'_annotation_core_unique_genes.txt',sep='\t',index=False)
...
def median_analysis():
	...
	for col in df_groupby2.columns[1:]:
		...
		df_temp['Orthogroup']=df_groupby2['Orthogroup']
		df_temp['genome_name']=str(col)
		df_temp['genome_name_Orthogroup']=df_temp['genome_name']+'_'+df_temp['Orthogroup']
		...
		df_concat['genome_name_Orthogroup']=df_concat['genome_name']+'_'+df_concat['Orthogroup']
		#annotation metadata
		df_merged=pd.merge(df_temp,df_concat,left_on='genome_name_Orthogroup',right_on='genome_name_Orthogroup',how="left").rename(columns={'genome_name_y': 'genome_name','Orthogroup_x':'Orthogroup'}).drop(['Orthogroup_y','genome_name_x','genome_name_Orthogroup','count'],axis=1)
		col_order=['genome_name','feature_id','pgfam','Orthogroup','gene_count','median','gene_count-median','vs_median']
...
def subgroup_genes():
	...
	df_metadata=pd.read_csv(str(args.metadata_file),sep='\t', dtype=object).replace(' ','_', regex=True)
	...
	for genome in genome_name_list:
		df_an=pd.read_csv(str(args.input_folder)+'/'+str(genome)+'_OG_annotation.txt',sep='\t')
		df_merged=pd.merge(df_an,df_concat,left_on='Orthogroup',right_on='Orthogroup',how="left").to_csv(output_folder+'/'+str(genome)+'_annotation_subgroup_genes.txt',sep='\t',index=False)
