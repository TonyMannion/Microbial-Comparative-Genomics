...
parser.add_argument('-i', '--features_file', dest='features_file', default = 'features.txt', help='Input desired name for features file for genomes of interest downloaded from PATRIC.  Use a tab-delimited file')
...
df1['count']=1
df2 = df1.replace(' ', '_', regex=True).groupby([str(args.index_column), str(args.count_column)], as_index=False).sum().pivot(index = str(args.index_column), columns = str(args.count_column), values = 'count').fillna(0).astype(int)
for col in df2:
	df2[col] = np.where(df2[col] > 0, 1, 0) # If value is greater than 0, replace with 1, otherwise replace with 0.  Necessary because genome may have more than one count for protein family.
total_rows=len(df2.axes[0])
total_cols=len(df2.axes[1])
df2.to_csv(str(args.output), sep='\t') 
with open(str(args.output), "rb") as f:
	lines = f.readlines()
