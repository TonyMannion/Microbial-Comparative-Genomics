...
parser.add_argument('-c', '--contigs_table', dest='contigs_table', default = 'contigs_table.txt', help='Metadata tab-delimited file for contig file names and genome names. Default is conitgs_table.txt')
parser.add_argument('-u', '--upload_contigs', dest='upload_contigs', default = 'yes', help='Upload contigs files? Enter yes or no. Default is yes')
parser.add_argument('-a', '--annotate', dest='annotate', default = 'yes', help='Execute annotate pipeline? Enter yes or no. Default is yes', )
parser.add_argument('-d', '--download_reports', dest='download_reports', default = 'yes', help='Download genome reports, contigs, and annotations data.  Note: cannot execute blast unless annotate pipeline has been previously performed', )
...
df = pd.read_csv(str(args.contigs_table), sep='\t')
contigs_list = df['contigs'].tolist()
genome_name_list = df['genome_name'].tolist()

if str(args.upload_contigs) == 'yes':
