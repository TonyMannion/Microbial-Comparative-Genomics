...
def core_unique_genes(input_file):
	...
	df2 = df.iloc[:,[0,-1]].to_csv('temp_core_filter_out.txt', sep='\t',index=False)
	...
	for genome in genome_name_list:
		...
		df2 = df.iloc[:,[0,-1]].to_csv('temp_'+str(genome) + '_unique_filter_out.txt', sep='\t',index=False)
	#concat
	temp_filter_files = glob.glob('temp_' + '*_filter_out.txt')
	...
	#merge
	for genome in genome_name_list:
		df_an = pd.read_csv(str(genome)+'_annotation.txt', sep='\t')
		df_merged = pd.merge(df_an, df_concat, left_on='pgfam', right_on='pgfam', how="left").to_csv(str(genome)+'_annotation.txt',  sep='\t', index=False)
...
if str(args.downlaod_patric_features) == 'yes':
	...
	for genome_id, genome_name in zip(genome_ids_list,genome_name_list): 
		...
		df_genome = pd.read_csv(str(genome_name) +'_annotation.txt', sep='\t')
		df_genome2=df_genome.rename(columns={'feature.genome_name': 'genome_name','feature.pgfam_id': 'pgfam'})
		df_genome2.to_csv(str(genome_name) +'_annotation.txt', sep='\t', index=False)
...
if str(args.annotations) == 'yes':
	...
	if os.path.isfile('concatenated_annotations.txt'):
		...
		core_unique_genes('concatenated_annotations.txt')
	else:
		print 'making file'
		df_genome_names = pd.read_csv(str(args.metadata_file), sep='\t', usecols=['genome_name'])
		genome_name_list = df_genome_names['genome_name'].dropna().tolist()
