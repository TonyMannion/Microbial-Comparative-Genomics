...
for list in lists:
	...
	df = pd.read_csv('gene_family_groupby_out.txt', sep='\t') #output from gene_family_analysis.py
	for genome in genome_list_include:
		df=df[df[str(genome)]>0]
	for genome in genome_list_exclude:
		df= df[df[str(genome)]<=0]
	df['category']=str(list)
...
for genome in genome_list:
	...
	df_an = pd.read_csv(str(genome)+'_median_gene_analysis.txt', sep='\t') #output from gene_family_analysis.py
